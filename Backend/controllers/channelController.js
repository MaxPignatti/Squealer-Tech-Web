...
exports.deleteChannel = async (req, res) => {
	try {
		...
		// Trova il canale utilizzando l'ID fornito
		const channel = await Channel.findById(channelId);
		if (!channel) {
			return res.status(404).json({ message: "Canale non trovato" });
		}

		// Estrai il nome del canale trovato
		const channelName = channel.name;

		// 1. Elimino il canale dal database
		await Channel.findByIdAndDelete(channelId);

		// 2. Trovo e aggiorno i messaggi che contengono il canale, rimuovendolo dall'array `channel`
		await Message.updateMany(
			{ channel: channelName },
			{ $pull: { channel: channelName } }
		);

		// 3. Elimino i messaggi che ora non appartengono a nessun canale
		await Message.deleteMany({ channel: { $size: 0 } });

		// 4. Rimuovo il canale dal campo `channels` di tutti gli utenti iscritti
