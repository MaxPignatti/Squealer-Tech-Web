...
const Pro = () => {
    ...
    const handleProAction = async (action) => {
        try {
            const response = await fetch(`http://localhost:3500/usr/${userData.username}/proAction`, {
                method: "POST",
                headers: {
            ...
            console.error("API call error:", error);
            setErrorMessage("Si è verificato un errore durante la comunicazione con il server.");
        }
    };
        ...
        <div className="d-flex justify-content-center align-items-center">
            <Card style={cardStyle}>
                <Card.Body>
                    ...
                    {userData.isPro ? (
                        <Card.Text>Sei già Pro</Card.Text>
                    ) : isRequestingPro ? ( // Usa 'isRequestingPro' per determinare quale interfaccia mostrare
                        <>
                            <Card.Text>Hai già effettuato la richiesta</Card.Text>
                            <Button variant="danger" onClick={() => handleProAction("cancel")}>
                                Annulla Richiesta
                            </Button>
                        </>
                    ) : (
                        <>
                            <Card.Text>Con un account Pro potrai accedere a vantaggi esclusivi.</Card.Text>
                            <Button variant="primary" onClick={() => handleProAction("request")}>
                                Richiedi
                            </Button>
                        </>
                    )}
