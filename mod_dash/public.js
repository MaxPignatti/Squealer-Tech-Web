...
async function fetchAllMessages() {
	try {
		const response = await fetch(
			"https://site222327.tw.cs.unibo.it/api/allMessages"
		);
		if (!response.ok) {
...
async function fetchChannels() {
	try {
		const response = await fetch(
			"https://site222327.tw.cs.unibo.it/api/channels/moderator/false"
		);
		if (!response.ok) {
	...
	channels.forEach((channel) => {
		...
		card.innerHTML = `
            <div class="card-body">
				...
				<button onclick="updateChannel('${channel._id}', '${channel.name}', '${channel.description}')" class="btn btn-danger btn-sm" aria-label="Modifica canale ${channel.name}">Modifica Canale</button>
...
function renderMessagesForChannel(channelName, channelId) {
	...
	console.log(messagesForChannel, channelName);
	...
	messagesForChannel.forEach((message) => {
		...
		tr.innerHTML = `
			...
			<td>
			<button class="btn btn-danger btn-sm" onclick="editMessage('${
				message._id
